---
title: "第 10 課：將 Spring Boot 3.x 連接到 MySQL"
lesson: 10
slug: "lesson-10"
summary: "持久化資料是大多數後端系統的核心部分，而正確的資料庫設定就是基礎。"
---

# 第 10 課：將 Spring Boot 3.x 連接到 MySQL
...
當依賴存在之後，設定就成了關鍵步驟。Spring Boot 3.x 需要知道資料庫 URL、使用者名稱、密碼，以及很多情況下會影響 schema 處理與 SQL 行為的 JPA 或 Hibernate 相關設定。實務上，這通常代表建立在 Hibernate 6 之上的 Spring Data JPA，再加上 MySQL driver。
...
一旦 MySQL 正確連接上，後面的持久化故事才真正開始。entity、repository、transaction，以及 CRUD 行為都建立在這個基礎之上。這裡也值得建立一個健康習慣：像 `spring.jpa.hibernate.ddl-auto=update` 這種方便的設定，在本機開發可以接受，但正式環境通常會把 schema 變更交給 Flyway 或 Liquibase 這類專門的 migration 工具。
...
# 只建議作為本機開發的方便設定；正式環境通常改用 migration 工具。
