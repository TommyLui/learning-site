...
## What You Will Learn
- 學會如何使用官方 Initializr 流程建立乾淨的 Spring Boot 4.x 專案。
...
`start.spring.io` 這個頁面看起來很簡單，但每個欄位都控制著有意義的內容。Project 會決定建置工具，通常是 Maven 或 Gradle。Language 會決定產生的是 Java、Kotlin 還是 Groovy 程式碼。Spring Boot 則會選定你要對齊的 framework 版本。對你的學習路線來說，它應該與你正在整理的 4.x 分支保持一致。
...
## Practice
- 建立一個只選擇 `Spring Web` 的 Spring Boot 4.x 新專案。
